...
enum VSRFrame {
    ...
    static func decodeReply(header: ArraySlice<UInt8>, body: ArraySlice<UInt8>) throws -> ArraySlice<UInt8> {
        ...
        // A send the server classified as a duplicate of an earlier request is
        // answered with an empty status-0 body: the batch is already in the
        // log, so it reads as a success with no confirmations to report.
