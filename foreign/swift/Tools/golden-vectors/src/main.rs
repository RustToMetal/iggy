...
use iggy_binary_protocol::responses::messages::{
    PollMessagesResponseHeader, SendMessagesConfirmationResponse, SendMessagesResponse,
};
...
use iggy_binary_protocol::responses::system::{
    ClusterMetadataResponse, ClusterNodeResponse, DescribeOptionsResponse, OptionDescriptor,
};
...
use iggy_binary_protocol::responses::users::{
    GetUsersResponse, IdentityResponse, LoginRegisterResponse, UserDetailsResponse, UserResponse,
};
use iggy_binary_protocol::{
    AckLevel, ClientVersionInfo, IGGY_PROTOCOL_VERSION, IGGY_PROTOCOL_VERSION_MIN, WireConsumer,
...
fn main() {
    ...
    lengths.extend([
        511, 512, 513, 1000, 1023, 1024, 1025, 1279, 1280, 1281, 2047, 2048, 2049, 4096, 10_000,
        65_536, 100_003,
    ]);
    let xxh3_64 = lengths
        .iter()
        .map(|&len| (len, format!("{:016x}", XxHash3_64::oneshot(&pattern(len)))))
        .collect();
    let xxh32 = lengths
        .iter()
        .map(|&len| (len, format!("{:08x}", XxHash32::oneshot(0, &pattern(len)))))
        .collect();

    let errors = (1u32..=20_000)
        .filter_map(|code| {
            let error = IggyError::from_code(code);
            (error.as_code() == code).then(|| (code, error.as_string().to_owned()))
        })
        .collect();
    ...
    add(
        &mut vectors,
        "identifier.named.my-stream",
        &named("my-stream"),
    );
    add(
        &mut vectors,
        "consumer.numeric.42",
        &WireConsumer::consumer(numeric(42)),
    );
    add(
        &mut vectors,
        "consumer_group.named.my-group",
        &WireConsumer::consumer_group(named("my-group")),
    );
    add(
        &mut vectors,
    ...
    add(
        &mut vectors,
        "partitioning.partition_id.7",
        &WirePartitioning::PartitionId(7),
    );
    add(
        &mut vectors,
        "partitioning.messages_key.user-123",
        &WirePartitioning::MessagesKey(b"user-123".to_vec()),
    );
    add(
        &mut vectors,
        "polling.offset.100",
        &WirePollingStrategy::offset(100),
    );
    add(
        &mut vectors,
        "polling.timestamp.1700000000000",
        &WirePollingStrategy::timestamp(1_700_000_000_000),
    );
    ...
    add(
        ...
        &CreateStreamRequest {
            name: name("test-stream"),
            options: WireOptions::empty(),
        },
    );
    add(
        &mut vectors,
        "request.create_stream.options",
        &CreateStreamRequest {
            name: name("test-stream"),
            options: sample_options(),
        },
    ...
    add(
        ...
        &UpdateTopicRequest {
            stream_id: numeric(1),
            topic_id: numeric(2),
            name: name("updated-topic"),
            options: options(&[(2, b"message_expiry", 12, &604_800_000_000u64.to_le_bytes())]),
        },
    );
    add(
        &mut vectors,
        "request.create_partitions",
    ...
    add(
        &mut vectors,
        "request.delete_partitions",
        &DeletePartitionsRequest {
            stream_id: named("stream"),
            topic_id: named("topic"),
            partitions_count: 2,
        },
    );
    add(
    ...
    add(
        &mut vectors,
        "request.get_consumer_group",
        &GetConsumerGroupRequest {
            stream_id: named("stream-1"),
            topic_id: named("topic-1"),
            group_id: named("group-1"),
        },
    );
    add(
        &mut vectors,
        "request.get_consumer_groups",
    ...
    add(
        &mut vectors,
        "request.store_consumer_offset",
        &StoreConsumerOffsetRequest {
            consumer: WireConsumer::consumer(numeric(1)),
            stream_id: numeric(10),
            topic_id: numeric(20),
            partition_id: Some(5),
            offset: 12_345,
            ack: AckLevel::Quorum,
    ...
    add(
        &mut vectors,
        "request.store_consumer_offset.no_partition",
        &StoreConsumerOffsetRequest {
            consumer: WireConsumer::consumer_group(numeric(3)),
            stream_id: numeric(1),
            topic_id: numeric(1),
            partition_id: None,
            offset: u64::MAX,
    ...
    add(
        &mut vectors,
        "request.get_consumer_offset",
        &GetConsumerOffsetRequest {
            consumer: WireConsumer::consumer(named("my-consumer")),
            stream_id: named("stream-1"),
            topic_id: named("topic-1"),
            partition_id: Some(0),
        },
    ...
    add(
        &mut vectors,
        "request.delete_consumer_offset",
    ...
    add(
        &mut vectors,
        "request.flush_unsaved_buffer",
        &FlushUnsavedBufferRequest {
            stream_id: numeric(1),
            topic_id: numeric(2),
            partition_id: 3,
            fsync: true,
    ...
    add(
        &mut vectors,
        "request.create_user.no_permissions",
        &CreateUserRequest {
            username: name("user"),
    ...
    add(
        &mut vectors,
        "request.update_permissions.none",
        &UpdatePermissionsRequest {
            user_id: named("admin"),
            permissions: None,
        },
    ...
    add(
        ...
        &ChangePasswordRequest {
            user_id: numeric(1),
            current_password: "old-pass".to_owned(),
            new_password: "new-pass-123".to_owned(),
        },
    );
    add(
        &mut vectors,
        "request.login_register",
    ...
    add(
        &mut vectors,
        "request.login_register_with_pat",
    ...
    add(
        &mut vectors,
        "request.create_personal_access_token",
    ...
    add(
        &mut vectors,
        "request.delete_personal_access_token",
    ...
    add(
        &mut vectors,
        "request.get_personal_access_tokens",
    ...
    add(
        &mut vectors,
        "request.get_client",
        &GetClientRequest { client_id: 42 },
    );
    add(&mut vectors, "request.get_clients", &GetClientsRequest);
    add(
        &mut vectors,
        "request.get_cluster_metadata",
        &GetClusterMetadataRequest,
    );
    add(
        &mut vectors,
    ...
    add(
        &mut vectors,
        "request.describe_options",
        &DescribeOptionsRequest { scope: 1 },
    );
    ...
    // Send messages: the full body a producer writes.
    {
        ...
        encode_user_headers(
            &[
                (2, b"attempt", 11, &3u32.to_le_bytes()),
                (2, b"trace-id", 2, b"abc"),
            ],
            &mut headers,
        );
        ...
        let messages = [
            ...
            RawMessage {
                id: 2,
                origin_timestamp: 1_500,
                headers: Some(&headers),
                payload: b"second",
            },
            RawMessage {
                id: 0xDEAD_BEEF_0000_0000_0000_0000_0000_0003,
                origin_timestamp: 1_000,
                headers: None,
                payload: b"",
            },
    ...
    // Consensus headers.
    let create_stream_payload = CreateStreamRequest {
        name: name("stream"),
        options: WireOptions::empty(),
    }
    .to_bytes();
    vectors.insert(
        "payload.create_stream".to_owned(),
        hex(&create_stream_payload),
    );
    let client_id: u128 = 0x0123_4567_89AB_CDEF_FEDC_BA98_7654_3210;
    vectors.insert(
        "vsr.request.register".to_owned(),
        hex(bytemuck::bytes_of(&request_header(
            LOGIN_REGISTER_CODE,
            client_id,
            0,
            0,
    ...
    vectors.insert(
        "vsr.request.ping.bound".to_owned(),
        hex(bytemuck::bytes_of(&request_header(
            PING_CODE,
            client_id,
            3,
    ...
    vectors.insert(
        "vsr.request.create_stream".to_owned(),
        hex(bytemuck::bytes_of(&request_header(
            CREATE_STREAM_CODE,
            client_id,
            1,
    ...
    vectors.insert(
        "vsr.request.send_messages".to_owned(),
        hex(bytemuck::bytes_of(&request_header(
            SEND_MESSAGES_CODE,
            client_id,
            2,
    ...
    vectors.insert(
        "vsr.request.unknown_code".to_owned(),
        hex(bytemuck::bytes_of(&request_header(
            60_000,
            client_id,
            5,
    ...
    vectors.insert(
        "vsr.reply.denied".to_owned(),
        hex(bytemuck::bytes_of(&denied)),
    );
    let eviction = EvictionHeader::new(0, 0, 0, client_id, EvictionReason::StaleClient);
    vectors.insert(
        "vsr.eviction.stale_client".to_owned(),
        hex(bytemuck::bytes_of(&eviction)),
    );
    let incompatible = EvictionHeader::incompatible_protocol(
        0,
        0,
        0,
        client_id,
    ...
    vectors.insert(
        "vsr.eviction.incompatible_protocol".to_owned(),
        hex(bytemuck::bytes_of(&incompatible)),
    );
    ...
    add(
        ...
        "response.get_stream",
        &GetStreamResponse {
            stream: stream_response(1, 2),
            topics: vec![topic_header(1, "topic-a"), topic_header(2, "topic-b")],
        },
    );
    add(
        &mut vectors,
        ...
        &GetStreamsResponse {
            streams: vec![stream_response(1, 2), stream_response(2, 0)],
        },
    );
    add(
        &mut vectors,
        "response.topic_header",
        &topic_header(5, "events"),
    );
    let partitions = vec![
        PartitionResponse {
            id: 0,
    ...
    add(
        ...
        "response.get_topics",
        &GetTopicsResponse {
            topics: vec![topic_header(1, "events"), topic_header(2, "logs")],
        },
    );
    add(
        &mut vectors,
        "response.get_topics.empty",
        &GetTopicsResponse { topics: vec![] },
    );
    add(
        &mut vectors,
    ...
    add(
        &mut vectors,
        "response.consumer_group_details",
    ...
    add(
        &mut vectors,
        "response.client_details.no_user",
        &ClientDetailsResponse {
            client: ClientResponse {
                client_id: 2,
                user_id: u32::MAX,
                transport: 1,
                address: "10.0.0.1:6000".to_owned(),
    ...
    add(
        &mut vectors,
        "response.user_details.no_permissions",
        &UserDetailsResponse {
            user: user.clone(),
            permissions: None,
        },
    ...
    add(
        &mut vectors,
        "response.login_register",
        &LoginRegisterResponse {
            user_id: 42,
            session: 100,
            server_protocol_version: IGGY_PROTOCOL_VERSION,
            server_version: name("0.11.0"),
        },
    );
    add(
        &mut vectors,
        "response.identity",
        &IdentityResponse { user_id: 42 },
    );
    add(
        &mut vectors,
        "response.raw_personal_access_token",
        &RawPersonalAccessTokenResponse {
            token: name("raw-secret-token-value"),
        },
    );
    add(
        &mut vectors,
        "response.get_personal_access_tokens",
        &GetPersonalAccessTokensResponse {
            tokens: vec![
                PersonalAccessTokenResponse {
                    name: name("token-a"),
                    expiry_at: 1_710_000_000_000_000,
                },
                PersonalAccessTokenResponse {
                    name: name("token-b"),
                    expiry_at: 0,
    ...
    add(
        &mut vectors,
        "response.describe_options",
        &DescribeOptionsResponse {
            entries: vec![
                OptionDescriptor {
                    key: name("segment_size"),
                    kind: 12,
                    default_value: Bytes::copy_from_slice(&1_073_741_824u64.to_le_bytes()),
                    description: "Segment size".to_owned(),
                },
                OptionDescriptor {
                    key: name("durability"),
                    kind: 2,
    ...
    // Poll response: header + two batch records.
    {
        let first = batch_record(
            ...
            &[
                frame(11, 0, 0, b"a", &[]),
                frame(12, 1, 10, b"b", &vectors_user_headers()),
            ],
        );
    ...
    add(
        &mut vectors,
        "options.topic_create.full",
        &create_options.to_wire().expect("encodes"),
    );
    ...
    add(
        &mut vectors,
        "options.topic_update.full",
        &update_options.to_wire().expect("encodes"),
    );
    let mut raw = BTreeMap::new();
    raw.insert("durability".to_owned(), "persisted".to_owned());
    let raw_options = TopicCreateOptions {
        raw,
        ..Default::default()
    };
    add(
        &mut vectors,
        "options.topic_create.raw",
        &raw_options.to_wire().expect("encodes"),
    );
    ...
    let mut json = serde_json::to_string_pretty(&golden).expect("serializes");
    json.push('\n');
    if let Some(parent) = std::path::Path::new(&output).parent() {
        std::fs::create_dir_all(parent).expect("creates the output directory");
    }
