...
struct RequestVectorTests {
    ...
    func encode(_ id: Identifier) throws -> [UInt8] {
        try Requests.identifierOnly(id)
    }

    func encodeConsumer(_ consumer: Consumer) throws -> [UInt8] {
    ...
    var sampleOptions: ResourceOptions {
        ["durability": .explicit("persisted"), "segment_size": .explicit(.uint64(1_073_741_824))]
    }
    ...
    @Test func primitives() throws {
        ...
        #expect(try encodeConsumer(.consumer(42)) == golden.bytes("consumer.numeric.42"))
        #expect(try encodeConsumer(.group("my-group")) == golden.bytes("consumer_group.named.my-group"))
        ...
        var writer = ByteWriter()
        try Requests.versionInfo(into: &writer)
    ...
    @Test func streams() throws {
        ...
        #expect(try encode(5) == golden.bytes("request.delete_stream"))
        #expect(try encode("my-stream") == golden.bytes("request.get_stream"))
        #expect(golden.bytes("request.get_streams").isEmpty)
        #expect(try encode(1) == golden.bytes("request.purge_stream"))
    ...
    @Test func topics() throws {
        ...
        #expect(try Requests.streamAndTopic(1, 5) == golden.bytes("request.delete_topic"))
        #expect(try Requests.streamAndTopic("my-stream", "my-topic") == golden.bytes("request.get_topic"))
        #expect(try encode(42) == golden.bytes("request.get_topics"))
        #expect(try Requests.streamAndTopic(1, 3) == golden.bytes("request.purge_topic"))
    ...
    @Test func partitionsAndSegments() throws {
        #expect(try Requests.partitions(streamID: 1, topicID: 2, partitionsCount: 5) == golden.bytes("request.create_partitions"))
        #expect(try Requests.partitions(streamID: "stream", topicID: "topic", partitionsCount: 2) == golden.bytes("request.delete_partitions"))
        #expect(try Requests.deleteSegments(streamID: 1, topicID: 2, partitionID: 3, segmentsCount: 10) == golden.bytes("request.delete_segments"))
    }
    ...
    @Test func consumerGroups() throws {
        ...
        #expect(try Requests.consumerGroup(streamID: 1, topicID: 2, groupID: 3) == golden.bytes("request.delete_consumer_group"))
        #expect(try Requests.consumerGroup(streamID: "stream-1", topicID: "topic-1", groupID: "group-1") == golden.bytes("request.get_consumer_group"))
        #expect(try Requests.streamAndTopic(1, 2) == golden.bytes("request.get_consumer_groups"))
        #expect(try Requests.consumerGroup(streamID: 1, topicID: 2, groupID: 3) == golden.bytes("request.join_consumer_group"))
        #expect(try Requests.consumerGroup(streamID: 1, topicID: 2, groupID: "g") == golden.bytes("request.leave_consumer_group"))
        #expect(try Requests.consumerGroup(streamID: "stream-1", topicID: "topic-1", groupID: "group-1") == golden.bytes("request.sync_consumer_group"))
    }

    @Test func consumerOffsets() throws {
        #expect(
            try Requests.storeConsumerOffset(consumer: .consumer(1), streamID: 10, topicID: 20, partitionID: 5, offset: 12_345, ack: .quorum)
                == golden.bytes("request.store_consumer_offset"))
        #expect(
            try Requests.storeConsumerOffset(consumer: .group(3), streamID: 1, topicID: 1, partitionID: nil, offset: UInt64.max, ack: .noAck)
                == golden.bytes("request.store_consumer_offset.no_partition"))
        #expect(
            try Requests.getConsumerOffset(consumer: .consumer("my-consumer"), streamID: "stream-1", topicID: "topic-1", partitionID: 0)
                == golden.bytes("request.get_consumer_offset"))
        #expect(
            try Requests.deleteConsumerOffset(consumer: .consumer(1), streamID: 10, topicID: 20, partitionID: 5, ack: .quorum)
                == golden.bytes("request.delete_consumer_offset"))
    }

    @Test func messages() throws {
        #expect(
            try Requests.pollMessages(consumer: .consumer(1), streamID: 10, topicID: 20, partitionID: 5, strategy: .offset(100), count: 50, autoCommit: true)
                == golden.bytes("request.poll_messages"))
        #expect(
            try Requests.pollMessages(consumer: .group(3), streamID: 1, topicID: 1, partitionID: nil, strategy: .first, count: 10, autoCommit: false)
                == golden.bytes("request.poll_messages.group"))
        #expect(try Requests.flushUnsavedBuffer(streamID: 1, topicID: 2, partitionID: 3, fsync: true) == golden.bytes("request.flush_unsaved_buffer"))
    ...
    @Test func users() throws {
        ...
        #expect(try encode("old-user") == golden.bytes("request.delete_user"))
        #expect(try encode(42) == golden.bytes("request.get_user"))
        ...
        #expect(try Requests.updatePermissions(userID: 1, permissions: samplePermissions) == golden.bytes("request.update_permissions"))
        #expect(try Requests.updatePermissions(userID: "admin", permissions: nil) == golden.bytes("request.update_permissions.none"))
